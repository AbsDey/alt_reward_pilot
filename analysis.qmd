...
if(length(unique_ids) == length(unique_sids)){
  msg = paste("ID formats match in number", length(unique_sids), ".\n\n")
} else{
  msg = paste("**WARNING**: ID formats do not match in number (long = ",
  length(unique_ids), "; short = ", length(unique_sids),
  ").\nConsider increasing the length of the short ID format.\n", sep="")
}
...
expected_count <- 1056 # minnimum count, multiple learning tries will increase this count
...
# 
# sum(ifelse(df$BlockType=="RewardList" & df$Word==df$rewardedColor & df$Congruency=="Incongruent", 1, 0), na.rm=T)
# 
# peeking<-df|>
#   filter(BlockType =="RewardList" & df$Word==df$rewardedColor)

df$rewarded_distractor_identity <- as.factor(ifelse(df$Word==df$rewardedColor,
                                                    "Yes", "No"))
...
p <- ggplot(df[df$data_exists, ], aes(x = joint_trial_num)) +
  ...
  # geom_point(aes(y = (Stimulus.RT/1000), color = Congruency, alpha = rewarded_this_cycle)) +
...
![](participant_trial_structure.svg){.lightbox}
:::
::::

## Filters

**Note:** Removed participant " 6837a5" due to them pressing any button as fast as possible (\<100 ms) during non-reward contexts
...
![](participant_rt_dist.svg){.lightbox}

## Alternating Blocks

### Reward Type x Congruency (all trials): RT
...
### Reward Type x Congruency (all trials): Errors
...
### Rewarded Color (by cycle) x Reward Block x Congruency: RT
...
### Distractor Identity Rewarded (by cycle) x Reward Block x Congruency: RT
...
### Trial Series: Color Item that was Rewarded or Rewarded Last Block

#### within person z-scored
...
p <- ggplot(data = rtc_series_summary, aes(x = rtc_trialnum, y = mean,
  ...
  scale_color_manual(values = c("Congruent" = "lightgreen",
                               "Incongruent" = "lightcoral")) +
  scale_x_continuous(breaks = seq(0, max(rtc_series_summary$rtc_trialnum), by = 5)) +
  labs(x = "Rewarded Trial Number",
       y = "Reaction Time (scaled)",
       title = "RT Series for only Rewarded Color by Congruency") +
  theme_classic() +
...
rtc_stroop_summary <- data |>
  ...
  mutate(scaled_rt = scale(Stimulus.RT)) |>
  ungroup() |>
  # Pivot wider to get congruent and incongruent in separate columns
  pivot_wider(
    id_cols = c(ShortID, rtc_trialnum),
    names_from = Congruency,
    values_from = scaled_rt
  ) |>
  mutate(stroop = Incongruent - Congruent) |>
  # Group by trial number to get means
  group_by(rtc_trialnum) |>
  summarise(
    mean_stroop = round(mean(stroop, na.rm = TRUE), 3),
    sd_stroop = round(sd(stroop, na.rm = TRUE), 3),
    n = n(),
    .groups = "drop"
  ) |>
  mutate(se_stroop = round(sd_stroop/sqrt(n), 3))
...
p <- ggplot(data = rtc_stroop_summary, aes(x = rtc_trialnum, y = stroop_scaled)) +
  ...
  geom_point(size = 3) +
  geom_line(size = 1) +
  scale_x_continuous(breaks = seq(0, max(rtc_stroop_summary$rtc_trialnum), by = 5)) +
  labs(x = "Rewarded Trial Number",
       y = "Stroop (scaled)",
       title = "Stroop Series for only Rewarded Color") +
  theme_classic() +
  theme(axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14),
        axis.text.x = element_text(size = 12),
...
![](rewarded_color_series.svg){.lightbox}

![](stroop_series.svg){.lightbox}
...
for (id in unique_sids) {
  ...
  cat("<br>")
  cat("con cutoff = ", con_cutoff, "inc cutoff = ", inc_cutoff ,"\n\n")
  print(
    response_digit_df |>
      filter(ShortID == id) |>
      select(-ShortID) |>
