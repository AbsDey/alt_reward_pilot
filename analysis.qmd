...
1.  Reward Blocks — A specific ink color is rewarded

2.  No Reward Blocks — No colors/trials are rewarded.
...
### Trial Rewarded (collapsed blocks) x Congruency: RT
...
### Word Identity Rewarded (across cycle) x Reward Block x Congruency: RT
#### Caution: Both Color rewarded and not rewarded trials contaminate incongruent word-identity not rewarded trials
...
print(
  d_cycle_rt_overall |>
    rename(
      "Block Type" = BlockType,
      "Word Identity Rewarded (cycle)" = rewarded_distractor_identity,
      "Mean RT (ms)" = means,
      "SD" = sd,
      "N" = n
      ) |>
...
p <- ggplot(d_cycle_rt_overall, aes(x=Congruency, y=means,
  ...
  labs(color = "Word Identity Rewarded (cycle)", shape = "Word Identity Rewarded (cycle)") +
  theme_classic() +
  theme(axis.title.x = element_text(size=15),
        axis.title.y = element_text(size=15),
        axis.text.x = element_text(size = 15, color = "black"),
...
### Color-Word Identity Rewarded (across cycle) x Reward Block x Congruency: RT
...
#| fig-format: "svg"
#| fig-retina: 2
#| label: overall-color-distractor-reward-means

reward_color_distractor_rt_summary <- data |>
  filter(!is.na(AlternatingBlocks) & Stimulus.ACC == 1) |>
  droplevels() |>
  group_by(ShortID, BlockType, rewarded_this_cycle,
           rewarded_distractor_identity, Congruency) |>
  summarise(
    mean_rt = round(mean(Stimulus.RT, na.rm = TRUE)),
    sd_rt = round(sd(Stimulus.RT, na.rm = TRUE)),
    n = n(), 
...
reward_color_distractor_rt_overall <- reward_color_distractor_rt_summary |>
  group_by(BlockType, rewarded_this_cycle,
           rewarded_distractor_identity, Congruency) |>
  summarise(
    means = round(mean(mean_rt, na.rm = TRUE)),
    sd = round(sd(mean_rt, na.rm = TRUE)),
...
reward_color_distractor_rt_overall <- reward_color_distractor_rt_overall |>
  mutate(
    plot_shape = factor(
      ifelse(rewarded_distractor_identity == "Yes" & Congruency == "Incongruent", 1, 0),
      levels = c(0, 1),
      labels = c("Other", "Word Identity not Color Rewarded")
    )
  )


no_reward_cong <- reward_color_distractor_rt_overall |>
  filter(rewarded_this_cycle == "No", rewarded_distractor_identity == "No", Congruency == "Congruent")
reward_distractor_inc <- reward_color_distractor_rt_overall |>
  filter(rewarded_this_cycle == "No", rewarded_distractor_identity == "Yes", Congruency == "Incongruent")
# Combine for plotting line
grey_circ_tri <- bind_rows(no_reward_cong, reward_distractor_inc)

p <- ggplot(reward_color_distractor_rt_overall, aes(
    x = Congruency, y = means,
    group = rewarded_this_cycle,
    color = rewarded_this_cycle,
    shape = plot_shape
  )) +
  ...
  geom_line(
    data = grey_circ_tri,
    aes(x = Congruency, y = means, group = 1),
    color = "darkgrey",
    linetype = "dashed",
    size = 1,
    inherit.aes = FALSE
  ...
  scale_shape_manual(
    values = c("Other" = 16, "Word Identity not Color Rewarded" = 17),
    breaks = "Word Identity not Color Rewarded",
    labels = c("Word Identity but not Color Rewarded")
  ) +
  labs(color = "Color rewarded", shape = NULL) +
  theme_classic() +
  theme(
    ...
    legend.box.background = element_rect(fill = "white", colour = "black", linewidth = .5),
...
ggsave(
  filename = "color_distractor_rewarded.svg", 
  plot = p,
  device = "svg",
  width = 8,  
  height = 5,  
...
![](color_distractor_rewarded.svg){.lightbox style="margin-left: 0;"}
...
#| echo: false
#| warnings: false
#| label: participant-color-distractor-reward-means
...
for (id in unique_sids) {
  ...
  cat(id)
  print(
    reward_color_distractor_rt_summary |>
      filter(ShortID == id) |>
      select(-ShortID) |>
      knitr::kable(
        format = "markdown",
...
### Color-Word Identity Rewarded (across cycle) x Reward Block x Congruency: Error
...
#| fig-format: "svg"
#| fig-retina: 2
#| label: overall-color-distractor-reward-error-means

reward_color_distractor_err_summary <- data |>
  filter(!is.na(AlternatingBlocks)) |>
  ...
  summarise(
    ...
    sd_err = round(sd(error, na.rm = TRUE), 3),
    n = n(),
    .groups = "drop"
  ) |>
  mutate(BlockType = factor(BlockType,
...
reward_color_distractor_err_overall <- reward_color_distractor_err_summary |>
  group_by(BlockType, rewarded_this_cycle,
           rewarded_distractor_identity, Congruency) |>
  summarise(
    means = round(mean(mean_err, na.rm = TRUE), 3),
    sd = round(sd(mean_err, na.rm = TRUE), 3),
    n = n(),
    .groups = "drop"
    ) |>
  mutate(se = round(sd/sqrt(n), 3))

reward_color_distractor_err_overall <- reward_color_distractor_err_overall |>
  mutate(
    plot_shape = factor(
      ifelse(rewarded_distractor_identity == "Yes" & Congruency == "Incongruent", 1, 0),
      levels = c(0, 1),
...
no_reward_cong <- reward_color_distractor_err_overall |>
  filter(rewarded_this_cycle == "No", rewarded_distractor_identity == "No", Congruency == "Congruent")
reward_distractor_inc <- reward_color_distractor_err_overall |>
  filter(rewarded_this_cycle == "No", rewarded_distractor_identity == "Yes", Congruency == "Incongruent")
# Combine for plotting line
grey_circ_tri <- bind_rows(no_reward_cong, reward_distractor_inc)

p <- ggplot(reward_color_distractor_err_overall, aes(
    x = Congruency, y = means,
    group = rewarded_this_cycle,
    color = rewarded_this_cycle,
    shape = plot_shape
...
ggsave(
  filename = "color_distractor_rewarded_err.svg", 
  plot = p,
  device = "svg",
  width = 8,  
  height = 5,  
...
![](color_distractor_rewarded_err.svg){.lightbox style="margin-left: 0;"} <br>
...
### Trial Series: Error and Error-Stroop

#### Color Item that was rewarded or previously rewarded in the last reward block

```{r}
#| code-fold: true
#| label: reward-color-error-series
#| fig-width: 30  # Increased width
#| fig-height: 6  # Increased height
#| dpi: 300
#| fig-format: "svg"
...
rtc_series_err_summary <- data |>
  filter(!is.na(AlternatingBlocks) & !is.na(rtc_trialnum)) |>
  mutate(error=1-Stimulus.ACC) |>
  group_by(Congruency, rtc_trialnum) |>
  summarise(
...
p <- ggplot(data = rtc_series_err_summary,
            aes(x = rtc_trialnum, y = mean,
                color = Congruency, group = Congruency)) +
  geom_rect(data = shade_regions,
            aes(xmin = xmin, xmax = xmax,
...
![](rewarded_color_error_series.svg){.lightbox}

<br>

```{r}
#| code-fold: true
#| label: reward-color-error-stroop-series
#| fig-width: 30  # Increased width
#| fig-height: 6  # Increased height
#| dpi: 300
#| fig-format: "svg"
...
p <- ggplot(data = rtc_err_stroop_summary, aes(x = rtc_trialnum, y = err_stroop)) +
  geom_rect(data = shade_regions,
            aes(xmin = xmin, xmax = xmax,
                ymin = -Inf, ymax = Inf,
                fill = fill),
...
![](err_stroop_series.svg){.lightbox}
