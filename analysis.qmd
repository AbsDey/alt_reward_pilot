...
### Reward x Congruency RT Summmaries
...
<div>
<details>
<summary style="cursor: pointer; padding: 10px; background-color: #f0f0f0; border-radius: 4px;">
Click to show/hide participant-specific details
</summary>
...
### Reward x Congruency Error Rate Summmaries
...
### Rewarded Color (by cycle) x Reward Block x Congruency RT Summmaries
...
# cycle_rt_summary$BlockType <- factor(cycle_rt_summary$BlockType,
#                                     levels = c("RewardList", "NoRewardList"),
...
p <- ggplot(data=cycle_rt_overall, aes(x=Congruency, y=means,
  ...
  # coord_cartesian(ylim=c(580, 760))+
  theme_classic() +
...
### Trial Series for only Colors that are Rewarded or were Previously Rewarded Last Block
...
for (id in unique_sids) {
  ...
  cat("\n\n#### ", id, "\n", 
      "con cutoff = ", con_cutoff, "inc cutoff = ", inc_cutoff ,"\n\n")
  print(
    response_digit_df |>
      filter(ShortID == id) |>
      select(-ShortID) |>
